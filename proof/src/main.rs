...
use laverna::strategy::{
    StrategyReport, TokenForce, aggregate_pillars, classify_route_token, synthesize_strategy,
};
            ...
            rt.block_on(async {
                    ...
                    if http {
                        ...
                        let pid_file = std::env::var("HOME")
                            ...
                            .unwrap_or_else(|_| std::path::PathBuf::from("/tmp/lai-assistant.pid"));
                        if let Ok(contents) = std::fs::read_to_string(&pid_file)
                            && let Ok(old_pid) = contents.trim().parse::<i32>() {
...
fn infer_number_units(
    ...
    numbers
        ...
        .map(|&(p, _)| {
            ...
            if !rem.is_empty()
                && let Some(u) = try_unit(&rem)
            {
                return Some(u);
            }
            for &j in &[p.wrapping_sub(1), p + 1] {
                if j < toks.len()
                    && let Some(u) = try_unit(&toks[j].to_lowercase())
                {
                    return Some(u);
                }
            }
...
fn try_inverse_pre(
    ...
    for &f in &cands {
        ...
        if missing.len() == 1
            && bindings.len() == f.inputs.len() - 1
            && let Some(x) = solve_inverse(f, &bindings, &missing[0], target)
        {
...
fn run_solve(query: &str, explain_binding: Option<ExplainBindingMode>) -> SolveResult {
    ...
    if numerical.len() >= 2
        && let Some(expr) = extract_arithmetic_expression(query)
    {
...
fn cmd_corpus_validate(dir: Option<&str>) {
    ...
    for id in ent_reg.list_seeds() {
        if let Some(seed) = ent_reg.get_seed_ref(id)
            && seed.description.trim().is_empty()
        {
...
fn solve_to_json(result: &SolveResult, include_trace: bool) -> Value {
    ...
    if include_trace && let Value::Object(o) = &mut value {
        o.insert(
...
fn run_companion_turn(
    ...
    use laverna::companion::{MemoryCommand, parse_memory_command, parse_recall_query};
...
fn cmd_athena_entity_reason(
    ...
    if let Some(ref formula) = seed.formula
        && let Some(f) = formula_reg.get(formula)
        && !have_vars.contains(&f.output)
    {
