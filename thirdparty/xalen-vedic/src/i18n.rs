...
pub fn nakshatra_deity(nak: Nakshatra, lang: Language) -> &'static str {
    match lang {
        ...
        | Language::Gujarati
        | Language::Punjabi
        | Language::Odia => nakshatra_deity_hi(nak),
...
pub fn tithi_name(tithi: &Tithi, lang: Language) -> &'static str {
    match lang {
        ...
        | Language::Gujarati
        | Language::Punjabi
        | Language::Odia => tithi_name_hi(tithi),
        Language::Spanish
        | Language::Portuguese
        | Language::French
        | Language::German
...
fn tithi_name_hi(t: &Tithi) -> &'static str {
    ...
    // Clamp: a `number: 0` deserialized from untrusted input underflows `- 1`.
    NAMES[(t.number.clamp(1, 30) - 1) as usize]
}
...
pub fn yoga_name(yoga: &Yoga, lang: Language) -> &'static str {
    match lang {
        ...
        | Language::Gujarati
        | Language::Punjabi
        | Language::Odia => yoga_name_hi(yoga),
        Language::Spanish
        | Language::Portuguese
        | Language::French
        | Language::German
...
fn yoga_name_hi(y: &Yoga) -> &'static str {
    ...
    // Clamp: a `number: 0` deserialized from untrusted input underflows `- 1`.
    NAMES[(y.number.clamp(1, 27) - 1) as usize]
}
...
pub fn karana_name(karana: &Karana, lang: Language) -> &'static str {
    match lang {
        ...
        | Language::Gujarati
        | Language::Punjabi
        | Language::Odia => karana_name_hi(karana),
        Language::Spanish
        | Language::Portuguese
        | Language::French
        | Language::German
...
fn karana_name_hi(k: &Karana) -> &'static str {
    ...
    // Clamp: an out-of-range `name_index` deserialized from untrusted input.
    NAMES[k.name_index.clamp(0, 10) as usize]
}
...
mod tests {
    ...
    fn planet_names_malayalam() {
        assert_eq!(
            planet_name(Planet::Sun, Language::Malayalam),
            "സൂര്യന്\u{200d}"
        );
    ...
    #[test]
    fn localized_tithi_yoga_karana_never_panic_on_invalid_number() {
        use crate::panchang::Paksha;
        // The localized name lookups (Hindi/Tamil/Telugu paths and their
        // fallbacks) index const tables the same way the English ones do. A
        // `number: 0` or out-of-range index deserialized from untrusted input
        // must not underflow / panic in ANY language. Exhaust the u8 range.
        for &lang in &Language::ALL {
            for n in 0..=u8::MAX {
                let tithi = Tithi {
                    number: n,
                ...
                assert!(
                    !tithi_name(&tithi, lang).is_empty(),
                    "empty tithi name for number {n} in {lang:?}"
                );
                let yoga = Yoga { number: n };
                assert!(
                    !yoga_name(&yoga, lang).is_empty(),
                    "empty yoga name for number {n} in {lang:?}"
                );
                let karana = Karana {
                    number: n,
                    name_index: n,
                ...
                assert!(
                    !karana_name(&karana, lang).is_empty(),
                    "empty karana name for name_index {n} in {lang:?}"
                );
            }
        }
    }
