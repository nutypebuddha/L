...
pub struct VarshaphalResult {
    ...
    pub year: u32,
    /// Approximate Julian Day of the solar return (mean-tropical-year linear
    /// estimate; not a true solar-longitude search — see [`solar_return_jd`]).
...
///
/// WARNING — this is NOT the true solar return. It adds a fixed *mean*
/// tropical year and ignores the Sun's non-uniform apparent motion (the
/// equation of the centre, driven by Earth's orbital eccentricity). The true
/// instant at which the Sun returns to its exact natal longitude can differ
/// from this estimate by up to roughly a day, and the per-year offsets do not
/// cancel, so the error does not stay bounded over many returns. For the
/// year-lord (weekday) and Muntha to be reliable, replace this with a true
/// solar-longitude search (Newton iteration on apparent geocentric longitude).
/// This linear form is provided only as a coarse placeholder.
