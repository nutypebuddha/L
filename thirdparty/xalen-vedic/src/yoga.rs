...
///
/// Jupiter must occupy the 1st, 4th, 7th, or 10th house
...
pub struct GrahaYuddha {
    /// The planet that wins the war, per a SIMPLIFIED static brightness order
    /// (Venus > Jupiter > Mars > Saturn > Mercury). This is a heuristic, NOT a
    /// computed magnitude/latitude comparison — actual victory in the classical
    /// texts depends on apparent magnitude and ecliptic latitude at the moment,
    /// which vary and are not evaluated here. See [`detect_graha_yuddha`].
    pub winner: String,
    /// The planet that loses the war (per the same simplified ordering).
    pub loser: String,
...
///
/// This is a simplified brightness ordering.
