...
/// now matches.
static RECONCILED_CATALOG: OnceLock<Vec<FixedStar>> = OnceLock::new();

/// Build (once) and return the reconciled catalog — validated Hipparcos data
/// preferred for every named star, curated open clusters (Pleiades, Praesepe)
/// kept verbatim as a fallback.
fn reconciled_catalog() -> &'static Vec<FixedStar> {
    RECONCILED_CATALOG.get_or_init(|| {
        CATALOG
            .iter()
            .map(|curated| match find_generated_for_curated(curated.name) {
